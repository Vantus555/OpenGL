...
int main() {
	...
	glm::vec3 translationA(200, 200, 0); // Начальное положение объекта А
	glm::vec3 translationB(600, 200, 0); // Начальное положение объекта В
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		{
			glm::mat4 view = glm::translate(glm::mat4(1.0), translationA);
			glm::mat4 mvp = proj * view;
			shader.Bind();
			shader.SetUniforMat4f("u_MVP", mvp);
			render.Draw(vao, ib, shader);
		}{
			glm::mat4 view = glm::translate(glm::mat4(1.0), translationB);
			glm::mat4 mvp = proj * view;
			shader.Bind();
			shader.SetUniforMat4f("u_MVP", mvp);
			render.Draw(vao, ib, shader);
		...
		ImGui::SliderFloat3("translation A", &translationA.x, 0.0, 960.0);
		ImGui::SliderFloat3("translation B", &translationB.x, 0.0, 960.0);
