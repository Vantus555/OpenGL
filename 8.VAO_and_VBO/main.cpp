...
int main() {
	...
	cout << glGetString(GL_VERSION) << endl;// версия OpenGL
	float rectangle[] = {
		-0.5, -0.5,
		0.5, -0.5,
		0.5, 0.5,
	...
	GLCall(glEnableVertexAttribArray(0));// Включаем идекс 0 в массиве VAO, там хрянятся данные вершин
	GLCall(glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, sizeof(float) * 2, 0));// Говорим какие данные
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Выбираем шейдер который участвует в рисовании
		GLCall(glUseProgram(program));
		// Включаем тот массив вершин, объекты, которого хотим нарисовать
		GLCall(glBindVertexArray(vao));
		// Выбираем буффер индексов для рисования
		GLCall(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ibo));
		// Отображение
